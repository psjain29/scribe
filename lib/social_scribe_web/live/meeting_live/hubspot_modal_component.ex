defmodule SocialScribeWeb.MeetingLive.HubspotModalComponent do
  ...
  def handle_event("toggle_suggestion", params, socket) do
    ...
    updated_suggestions =
      Enum.map(socket.assigns.suggestions, fn suggestion ->
        ...
        suggested_value = Map.get(values, suggestion.field, suggestion.suggested_value || "")
        ...
        |> Map.put(
          :has_change,
          normalize_value(suggestion.existing_value) != normalize_value(suggested_value)
        )
      end)
  ...
  def handle_event("apply_updates", _params, socket) do
    send(
      self(),
      {:apply_hubspot_updates, socket.assigns.suggestions, socket.assigns.selected_contact,
