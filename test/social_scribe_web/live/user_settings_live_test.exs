    ...
    test "displays connected Salesforce accounts", %{conn: conn, user: user} do
      _credential =
        salesforce_credential_fixture(%{
          user_id: user.id,
          external_account_id: "00Dxx:005xx-demo",
          email: "salesforce-linked@example.com"
        })

      {:ok, view, _html} = live(conn, ~p"/dashboard/settings")

      assert has_element?(view, "li", "ID: 00Dxx:005xx-demo")
      assert has_element?(view, "li", "(salesforce-linked@example.com)")
